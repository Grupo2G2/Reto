...
class Equipo_Usuario_model extends CI_Model{
	...
	public function obtener_equipo_usuario($datosEditar){

		$sql="SELECT * FROM Usuario u, TUsuario tu, Usuario_Modulo um, Modulo m, Curso cu, Centro ce, Ciclo ci, Reto r, Reto_Modulo rm, Equipo e, Equipo_Usuario eu WHERE u.ID_Usuario = um.ID_Usuario AND u.ID_TUsuario = tu.ID_TUsuario AND m.ID_Modulo = um.ID_Modulo AND cu.ID_Curso = ci.ID_Curso AND ce.ID_Centro = ci.ID_Centro AND ci.ID_Ciclo = m.ID_Ciclo AND e.ID_Equipo = eu.ID_Equipo AND r.ID_Reto = rm.ID_Reto AND m.ID_Modulo = rm.ID_Modulo AND u.ID_Usuario = eu.ID_Usuario AND r.ID_Reto = e.ID_Reto AND eu.ID_Equipo_Alumno = $datosEditar";
